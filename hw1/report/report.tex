...
\[
	\fbox{\includegraphics[width=0.5\textwidth]{options}}
\]
\mysubsection{1}{
	...
	\begin{itemize}
		\item The option is in-the-money if $S_T > K$
			\begin{enumerate}
				...
				\item $\sigma\sqrt{T}Z > \ln\left(\frac{K}{S_0}\right) - (r-q-\frac{1}{2}\sigma^2)T$
			...
			\end{enumerate}
	\end{itemize}
	$d_2(K)$ is the z-score of $S_T$ reaching $K$, and $N(d_2(K))$ is the probability of $S_T$ reaching $K$. Therefore:
	$$E^Q[1_A] = N(d2(K1)) - N(d2(K2))$$
	$$E^Q[1_B] = N(d2(K2)) - N(d2(K3))$$
	$$E^Q[1_C] = N(d2(K3)) - N(d2(K4))$$
